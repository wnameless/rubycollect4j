...
/**
 * 
 * {@link RubyContract} provides few interfaces to define what are the methods a {@link Enumerable}
 * and an {@link Enumerator} should have. It's only for RubyCollect4j to use internally.
 * 
 * @author Wei-Ming Wu
 * 
 */
final class RubyContract {
   ...
   * @param <N> the type of what kind of {@link Enumerator} should be used
  ...
  interface Enumerable<E, N extends Enumerator<?, ?, ?>, Z extends Enumerable<?, ?, ?>>
     ...
     * 
     * @return true if non-null and not Boolean.FALSE object is found, false otherwise
     */
    boolean anyʔ();
    ...
    /**
     * Chunks elements into entries. The key of entry is the result returned by the block. The value
     * of entry is a {@link RubyArray} which contains elements having the same result returned by
     * the block and aside to each other.
     * 
     * @param <S> the type of transformed elements
     * @param block to chunk elements
     * @return {@link Enumerator}
     */
    <S> N chunk(Function<? super E, ? extends S> block);

    /**
     * Chunks elements into entries. The value of entry is a {@link RubyArray}. This method creates
     * a new chunk whenever the block returns false.
     * 
     ...
     * 
     * @param <S> the type of transformed elements
     * @param block to take element into a List
     * @return {@link Enumerable}
     */
     ...
     * 
     * @param block to define elements to be counted
     * @return the total number
     */
    int count(Predicate<? super E> block);

    /**
     * Generates a sequence from first element to last element and so on infinitely.
     * 
     * @return {@link Enumerator}
     */
    N cycle();
    ...
    /**
     * Generates a sequence from start element to end element, repeat n times. Yields each element
     * to the block.
     * 
     * @param n times to repeat
    ...
    /**
     * Generates a sequence from start element to end element and so on infinitely. Yields each
    ...
    /**
     * Finds the first element which gets true returned by the block. Returns null if element is not
     * found.
     * 
    ...
    /**
     * Drops the first n elements until an element gets false returned by the block.
     * 
     * @param block to define which elements to be dropped
     * @return {@link Enumerable}
     */
    ...
    /**
     * Iterates each element and puts the element with n - 1 consecutive elements into a
     * {@link RubyArray}.
     * 
     ...
     * 
     * @param n number of consecutive elements
     * @param block to yield the List of consecutive elements
     */
     ...
     * 
     * @param n size of each slice
     * @return {@link Enumerator}
     */
    N eachSlice(int n);

    /**
     * Slices elements into {@link RubyArray}s with length n and yield them to the block.
     * 
     * @param n size of each slice
     * @param block to yield each slice
     */
     ...
     * 
     * @param <O> the type of transformed elements
     * @param obj an object O
     * @return {@link Enumerator}
     */
    ...
    /**
     * Returns the index of first element which gets true returned by the block. Returns null if
     * nothing is found.
     * 
     * @param block to check elements
    ...
    /**
     * Returns the index of the target element. Returns null if the target is not found.
     * 
    ...
    /**
     * Finds all elements which are matched by the regular expression and transforms them.
     * 
     * @param <S> the type of transformed elements
     * @param regex regular expression
     * @param block to transform elements
    ...
    /**
     * Finds all elements which are not matched by the regular expression and transforms them.
     * 
     * @param <S> the type of transformed elements
     * @param regex regular expression
     * @param block to transform elements
    ...
    /**
     * Puts elements with the same result S returned by the block into a Entry&#60;S,
     * RubyArray&#60;E&#62;&#62;y of a {@link RubyHash}.
     * 
     * @param <S> the type of transformed elements
     * @param block to group each element
     * @return {@link RubyHash}
     */
    <S> RubyHash<S, RubyArray<E>> groupBy(Function<? super E, ? extends S> block);
    ...
    /**
     * Assigns the first element as the initial value. Reduces each element with block, then assigns
     * the result back to initial value and so on.
     * 
     * @param block to reduce each element
     * @return element
     */
    E inject(BiFunction<E, E, E> block);

    /**
     * Reduces each element with block, then assigns the result back to initial value and so on.
     * 
     * @param <I> the type of transformed elements
    ...
    /**
     * Finds the max element compared by the Comparator. Returns null if elements are empty.
     * 
     ...
     * @return element or null
     */
    <S> E maxBy(Comparator<? super S> comp, Function<? super E, ? extends S> block);

    /**
     * Finds the max element for outputs transformed by the block. Returns null if elements are
     * empty.
     * 
     * @param <S> the type of transformed elements
     * @param block to transform elements
    ...
    /**
     * Finds the min element for outputs compared by the Comparator. Returns null if elements are
     * empty.
     * 
     * @param comp a Comparator
     * @return element or null
     ...
     * @return element or null
     */
    <S> E minBy(Comparator<? super S> comp, Function<? super E, ? extends S> block);

    /**
     * Finds the min element for outputs transformed by the block. Returns null if elements are
     * empty.
     * 
     * @param <S> the type of transformed elements
     * @param block to transform elements
    ...
    /**
     * Finds the min and max elements for outputs transformed by the block and compared by the
     * Comparator.
     * 
     * @param <S> the type of transformed elements
     * @param comp a Comparator
     ...
     * @return {@link RubyArray}
     */
    <S> RubyArray<E> minmaxBy(Comparator<? super S> comp, Function<? super E, ? extends S> block);
    ...
    /**
     * Checks if elements contain only one element beside null and Boolean.FALSE.
     * 
     * @return true if only one element beside null and Boolean.FALSE is found, false otherwise
     */
    boolean oneʔ();
    ...
    /**
     * Groups elements into {@link RubyArray}s and the last element of each {@link RubyArray} should
     * get true returned by the block.
     * 
     * @param block to check where to do slice
     * @return {@link Enumerator}
     */
     ...
     * 
     * @param regex to check where to do slice
     * @return {@link Enumerator}
     */
    N sliceAfter(String regex);

    /**
     * Groups elements into {@link RubyArray}s and the first element of each {@link RubyArray}
     ...
     * 
     * @return {@link RubyArray}
     * @throws IllegalArgumentException when any 2 elements are not comparable
     */
    RubyArray<E> sort();
    ...
    /**
     * Sorts elements by the ordering of outputs transformed by the block induced by the Comparator,
     * then puts them into a {@link RubyArray}.
     * 
     * @param <S> the type of transformed elements
     * @param comp a Comparator
     * @param block to transform elements
     * @return {@link RubyArray}
     */
    <S> RubyArray<E> sortBy(Comparator<? super S> comp, Function<? super E, ? extends S> block);

    /**
     * Sorts elements by the ordering of outputs transformed by the block induced by the Comparator
     * for S and applies the Comparator for E again before puts them into a {@link RubyArray}.
     * 
     * @param <S> the type of transformed elements
     ...
     * @return {@link RubyArray}
     */
    <S> RubyArray<E> sortBy(Comparator<? super E> comp1, Comparator<? super S> comp2,
        Function<? super E, ? extends S> block);

    /**
     * Sorts elements by the ordering of outputs transformed by the block, then puts them into a
     * {@link RubyArray}.
     * 
     * @param <S> the type of transformed elements
     * @param block to transform elements
     ...
     * 
     * @param block to process each element before adding
     * @return {@link BigDecimal}
     */
     ...
     * @return {@link BigDecimal}
     */
    BigDecimal sum(Number init, Function<? super Number, ? extends Number> block);
    ...
    /**
     * Creates a {@link RubyHash} by transforming each element into an {@link Entry}.
     * 
     * @return {@link RubyHash}
     */
    ...
    /**
     * Creates a {@link RubyHash} by transforming each 2 elements into an {@link Entry}.
     * 
     * @return {@link RubyHash}
     */
    ...
    /**
     * Groups elements which get the same indices among all other Lists into {@link RubyArray}s.
     * 
     * @param others a List of Iterable
     * @return {@link Enumerable}
     */
    Z zip(List<? extends Iterable<? extends E>> others);

    /**
     * Groups elements which get the same indices among all other Iterables into {@link RubyArray}s
     * and yields them to the block.
     * 
     * @param others a List of Iterables
     * @param block to yield zipped elements
     */
    void zip(List<? extends Iterable<? extends E>> others, Consumer<? super RubyArray<E>> block);
  }
