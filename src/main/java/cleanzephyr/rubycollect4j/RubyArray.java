...
public final class RubyArray<E> extends RubyArrayBase<E> {
  ...
  public RubyEnumerator<RubyArray<E>> repeatedCombination(int n) {
    RubyArray<RubyArray<E>> rp = new RubyArray();
    if (n < 0) {
      return new RubyEnumerator(rp);
    }
    if (n == 0) {
      return new RubyEnumerator(rp.push(new RubyArray()));
    }
    int[] counter = new int[n];
    repeatedCombinationLoop(counter, 0, list.size() - 1, (count) -> {
      RubyArray<E> c = new RubyArray();
